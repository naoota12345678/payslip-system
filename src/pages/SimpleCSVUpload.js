...
const SimpleCSVUpload = () => {
  ...
  const saveHeaderMappings = async (headers, mappingSettings) => {
    ...
    try {
      ...
      headers.forEach((header, index) => {
        ...
        // デバッグ情報
        console.log(`📊 項目${index}: ヘッダー="${header}" → 表示名="${itemName}" (${category})`);
      ...
      await setDoc(doc(db, 'csvMappings', userDetails.companyId), mappingDoc);
      console.log('✅ 新しい形式でのマッピング保存完了');
      
      // デバッグ情報を出力
      console.log('📊 保存されたマッピング統計:');
  ...
  const fetchMappingSettings = async () => {
    console.log('🔍 [fetchMappingSettings] 給与マッピング設定取得開始');
    ...
    try {
      // 給与専用マッピング設定を取得
      console.log('🔍 給与マッピング設定取得開始 - コレクション: csvMappings, companyId:', userDetails.companyId);
      
      // 給与マッピングコレクションから設定を取得
      const mappingRef = doc(db, 'csvMappings', userDetails.companyId);
      const mappingDoc = await getDoc(mappingRef);
      
      if (!mappingDoc.exists()) {
        console.log('❌ csvMappingsドキュメントが存在しません');
        return {
          employeeIdColumn: '',
          departmentCodeColumn: '',
          simpleMapping: {},
      ...
      console.log('📋 csvMappingsの生データ取得成功:', data);
      ...
      if (data.mainFields) {
        ...
        console.log('✅ 新形式(mainFields)から取得:', { 
          employeeIdColumn, 
          departmentCodeColumn,
      ...
      // 従業員設定を正しいフィールド名で取得
      console.log('🔍 [fetchMappingSettings] mainFields確認:', data.mainFields);
      
      if (data.mainFields) {
        console.log('🔍 [fetchMappingSettings] mainFields.employeeCode:', data.mainFields.employeeCode);
        console.log('🔍 [fetchMappingSettings] mainFields.departmentCode:', data.mainFields.departmentCode);
        
        // 従業員コード設定
        if (data.mainFields.employeeCode && data.mainFields.employeeCode.headerName) {
          employeeIdColumn = data.mainFields.employeeCode.headerName;
          console.log('✅ 従業員コード設定取得 (mainFields):', employeeIdColumn);
        } else {
          console.log('❌ mainFields.employeeCode.headerNameが設定されていません');
        }
        
        // 部門コード設定
        if (data.mainFields.departmentCode && data.mainFields.departmentCode.headerName) {
          departmentCodeColumn = data.mainFields.departmentCode.headerName;
          console.log('✅ 部門コード設定取得 (mainFields):', departmentCodeColumn);
        } else {
          console.log('❌ mainFields.departmentCode.headerNameが設定されていません');
        }
      } else {
        console.log('❌ mainFieldsが存在しません');
      }
        ...
        if (data.employeeMapping && !employeeIdColumn && !departmentCodeColumn) {
          ...
          console.log('✅ 従業員設定取得 (旧形式employeeMapping):', { employeeIdColumn, departmentCodeColumn });
        }
      
      console.log('🎯 最終取得結果:', { employeeIdColumn, departmentCodeColumn });

      // 項目設定（給与専用）
      console.log('📊 給与設定処理開始');
        ...
        if (data[category] && Array.isArray(data[category])) {
          ...
          data[category].forEach(item => {
            if (item.headerName && item.itemName) {
              ...
              console.log(`✅ ${category}から追加: ${item.headerName} → ${item.itemName} (${itemType})`);
            }
          });
        }
      });
      ...
      console.error('❌ 給与マッピング設定取得エラー:', error);
      return { 
        employeeIdColumn: '', 
        departmentCodeColumn: '',
        simpleMapping: {},
  ...
  const saveToFirestore = async () => {
    ...
    setUploading(true);
    try {
      console.log('✅ 給与明細データ保存開始:', csvData.length, '件');
      ...
      for (let i = 0; i < csvData.length; i++) {
        ...
        if (mappingSettings.departmentCodeColumn && rowData[mappingSettings.departmentCodeColumn]) {
          ...
          console.log(`🏢 [行${i + 1}] マッピング設定カラム "${mappingSettings.departmentCodeColumn}" から部門コード取得: "${departmentCode}"`);
        }
        
        // デバッグ用（最初の数行のみ）
        if (i < 3) {
          console.log(`🚨 [緊急デバッグ] 行 ${i + 1}: 検出された従業員ID='${employeeId}' (使用カラム: ${mappingSettings.employeeIdColumn})`);
          console.log(`🚨 [緊急デバッグ] 行 ${i + 1}: CSVヘッダーとマッピング設定の照合:`);
        ...
        try {
          if (employeeId) {
            ...
            console.log(`🔍 従業員検索: companyId="${userDetails.companyId}", employeeId="${employeeId}"`);
            console.log(`🔍 employeesコレクションでの検索結果: ${employeesSnapshot.size}件見つかりました`);
            
            if (!employeesSnapshot.empty) {
              const employeeData = employeesSnapshot.docs[0].data();
              userId = employeesSnapshot.docs[0].id; // employeeDocumentIdをuserIdとして使用
              console.log(`✅ 従業員発見: ID=${userId}, 名前=${employeeData.name || employeeData.displayName}`, {
              ...
              if (allEmployeesSnapshot.size > 0) {
                ...
                console.log('🔍 employeeNumberフィールドでも検索してみます...');
                ...
                console.log(`🔍 employeeNumber検索結果: ${employeeNumberSnapshot.size}件`);
                ...
                if (!employeeNumberSnapshot.empty) {
                  ...
                  console.log(`✅ employeeNumberで従業員発見: ID=${userId}`, employeeData);
                }
              } else {
                console.log('❌ この会社にemployeesデータが全く存在しません！');
              }
            }
          } else {
            console.warn(`❌ 従業員番号が空です`);
          }
        } catch (userQueryError) {
          console.error(`従業員ID ${employeeId} のユーザー検索エラー:`, userQueryError);
        }

        // 【デバッグ】保存前の重要な情報を確認
        console.log(`💾 保存データ構築: 行${i + 1}`, {
        ...
        if (mappingSettings.mainFields) {
          ...
          if (mappingSettings.mainFields.employeeName && mappingSettings.mainFields.employeeName.headerName) {
            ...
            console.log(`👤 従業員名取得: カラム "${mappingSettings.mainFields.employeeName.headerName}" → "${employeeName}"`);
          }
          ...
          if (mappingSettings.mainFields.paymentDate && mappingSettings.mainFields.paymentDate.headerName) {
            ...
            console.log(`📅 支給年月取得: カラム "${mappingSettings.mainFields.paymentDate.headerName}" → "${paymentDateValue}"`);
          }
          ...
          if (mappingSettings.mainFields.identificationCode && mappingSettings.mainFields.identificationCode.headerName) {
            ...
            console.log(`🔖 識別コード取得: カラム "${mappingSettings.mainFields.identificationCode.headerName}" → "${identificationCode}"`);
          }
        }

        // 給与明細データを構築（給与/賞与分離対応）
        const payslipData = {
          companyId: userDetails.companyId,
          ...
          payslipType: 'salary', // 給与専用
          uploadId: uploadData.id, // アップロードIDを追加
          createdAt: serverTimestamp(),
        ...
        if (!userId) {
          console.warn(`従業員番号 ${employeeId} に対応するユーザーが見つかりません。給与明細は保存されますが、従業員本人は閲覧できません。`);
        }
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
              <h1 className="text-2xl font-bold mb-6">給与CSVアップロード</h1>
      
      <div className="bg-blue-50 p-4 rounded-lg mb-6">
        <h2 className="text-lg font-semibold mb-2">📝 使い方</h2>
        <ol className="list-decimal pl-5 space-y-1 text-sm">
          <li><strong>給与種別を選択</strong>（給与・賞与で完全分離設計）</li>
          <li>CSVファイルを選択（1行目：ヘッダー、2行目以降：データ）</li>
          <li>プレビューでデータを確認</li>
          <li>「給与明細として保存」ボタンで完了</li>
        </ol>
        <div className="mt-3 p-3 bg-yellow-50 border-l-4 border-yellow-400">
          <p className="text-sm text-yellow-800">
            ...
            事前に「CSVマッピング設定」で各種別の設定を完了してください。
          </p>
        </div>
      </div>
        ...
        <div className="mb-6">
          ...
          <div className="mb-3 p-3 bg-blue-50 rounded">
            ...
            <div className="text-sm text-gray-700">
              <span className="font-medium">{paymentYear}年{paymentMonth}月</span> の給与明細 
