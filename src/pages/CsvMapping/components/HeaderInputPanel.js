          ...
          <>
            <div className="mb-4">
              ...
              <textarea
                ...
                className="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 border"
                placeholder="例: KY01 KY02 KY03 KY04 KY11_0 KY11_1 KY11_2 ..."
              ></textarea>
              <p className="mt-1 text-xs text-gray-500">
                KY項目リストを入力してください。各KY項目は組み込みの列定義（識別コード、部門コードなど）に順番に対応付けされます。
              </p>
              ...
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                ...
                <div className="text-xs text-blue-700 max-h-40 overflow-y-auto">
                  <table className="min-w-full">
                    <thead>
                      <tr>
                        <th className="py-1 text-left">KY項目</th>
                    ...
                    <tbody>
                      {systemColumns.slice(0, 10).map((col, idx) => (
                        <tr key={idx}>
                          <td className="py-1 pr-4">{`KY${idx < 10 ? '0' + idx : idx}`}</td>
                ...
                <p className="mt-2 text-xs text-blue-600">
                  入力したKY項目が上記の順番で列に対応付けられます。例えばKY01が識別コード、KY02が部門コードとして扱われます。
                </p>
              </div>
            </div>
...
KY01 KY02 KY03 KY04 KY11_0 ..."
              ></textarea>
              <p className="mt-1 text-xs text-gray-500">
                エクセルから、1行目（ヘッダー行）と2行目（KY項目または値が入った行）を一緒にコピーして貼り付けてください。
              ...
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                ...
                <p className="text-xs text-blue-600">
                  1. エクセルなどで、1行目がヘッダー行（識別コードなど）、2行目がKY項目（KY01など）になっているデータを選択<br />
                  2. 2行全体を選択してコピー<br />
                  3. 上のテキストエリアに貼り付け<br />
                ...
                <div className="mt-2 text-xs text-blue-700">
                  ...
                  KY行　　　：KY01 KY02 KY03<br />
                  ↓<br />
                  「識別コード※」と「KY01」がマッピング<br />
                  「部門コード」と「KY02」がマッピング<br />
                  「部門名」と「KY03」がマッピング
                </div>
              </div>
            </div>
