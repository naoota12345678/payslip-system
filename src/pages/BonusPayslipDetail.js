...
function BonusPayslipDetail() {
    ...
    const fetchPayslipData = async () => {
      ...
      try {
        ...
        if (currentMappingConfig) {
          const csvSettingsDoc = await getDoc(doc(db, "csvSettingsBonus", payslipData.companyId || userDetails.companyId));
  ...
  const fetchSameDatePayslips = async (companyId, paymentDateStr, currentPayslipId) => {
    ...
    try {
      ...
      if (!snapshot.empty) {
        ...
        console.log(`📋 同じ支払日の賞与明細: ${sameDate.length}件, 現在位置: ${index + 1}/${sameDate.length}`);
      }
    } catch (err) {
      console.error("同じ支払日の賞与明細取得エラー:", err);
    }
  };
  ...
  // 印刷ボタンのハンドラ
  const handlePrint = () => {
    console.log('🖨️ 印刷ボタンがクリックされました');
    try {
      const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
      console.log('📱 デバイス情報:', { isMobile, userAgent: navigator.userAgent });
      
      if (isMobile) {
        console.log('📱 モバイルデバイス検出 - 遅延後に印刷実行');
        setTimeout(() => {
          console.log('🖨️ モバイル印刷実行');
          window.print();
        }, 100);
      } else {
        console.log('💻 デスクトップデバイス - 即座に印刷実行');
        window.print();
      }
    } catch (error) {
      console.error('🚨 印刷エラー:', error);
      alert('印刷中にエラーが発生しました。もう一度お試しください。');
    }
  };
  ...
  const handleBack = () => {
    ...
    const basePath = userDetails?.role === 'admin' ? '/admin/bonus-payslips' : '/employee/bonus-payslips';
  ...
  const handlePrev = () => {
    if (currentIndex > 0 && sameDatePayslips[currentIndex - 1]) {
      ...
      const paymentDateStr = getPaymentDateStr();
      navigate(`/admin/bonus-payslips/${prevId}?paymentDate=${encodeURIComponent(paymentDateStr)}`);
    }
  };
  ...
  const handleNext = () => {
    if (currentIndex < sameDatePayslips.length - 1 && sameDatePayslips[currentIndex + 1]) {
      ...
      const paymentDateStr = getPaymentDateStr();
      navigate(`/admin/bonus-payslips/${nextId}?paymentDate=${encodeURIComponent(paymentDateStr)}`);
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-4 flex justify-between items-center print:hidden">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={handlePrint}
            onTouchStart={() => console.log('🤏 印刷ボタンタッチ開始')}
            onTouchEnd={() => console.log('🤏 印刷ボタンタッチ終了')}
            className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 min-h-[44px] min-w-[44px] touch-manipulation active:bg-blue-800 transition-colors flex items-center"
