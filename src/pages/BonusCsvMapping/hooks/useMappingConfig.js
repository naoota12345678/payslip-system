  ...
  useEffect(() => {
    const fetchMappingConfig = async () => {
      ...
      try {
        ...
        const newMappingDoc = await getDoc(doc(db, 'csvMappingsBonus', userDetails.companyId));
  ...
  const saveMappingConfig = async (config, validationFn) => {
    ...
    try {
      ...
      // csvMappings（カテゴリ配列形式）に保存
              batch.set(doc(db, 'csvMappingsBonus', userDetails.companyId), {
        ...newFormatData,
        // mainFieldsを確実に保存
  ...
  const resetMappingConfig = async (type = 'mapping') => {
    try {
      ...
      if (type === 'mapping') {
        ...
        // 新しい形式の設定を削除
        const newMappingRef = doc(db, 'csvMappingsBonus', userDetails.companyId);
