...
### 2. 0値表示制御機能
**2025年8月実装**: 項目ごとに0値の表示/非表示を制御する機能

#### 機能概要
- **デフォルト動作**: 0値項目は非表示
- **個別制御**: 項目ごとに「0値表示」チェックボックスで制御
- **過去明細への影響**: 現在の設定が過去明細にも適用される

#### 動作仕様
- **項目名**: 固定化（過去明細は変更されない）
- **0値表示**: 現在の設定を参照（過去明細も影響を受ける）
- **新規項目**: デフォルトで0値非表示

#### 設定方法
1. CSVマッピング設定画面を開く
2. 各項目の「0値表示」チェックボックスで制御
3. チェックあり: 0値でも表示
4. チェックなし: 0値は非表示（デフォルト）
...
### 2025年8月5日 - 0値表示制御機能実装
- **追加機能**:
  - 項目ごとの0値表示/非表示制御
  - CSVマッピング画面に「0値表示」チェックボックス追加
  - デフォルトで0値項目を非表示

- **変更ファイル**:
  - `src/components/payslip/PayslipPreview.js`: 0値フィルタリング実装
  - `src/pages/PayslipDetail.js`: showZeroValueプロパティ追加
  - `src/pages/BonusPayslipDetail.js`: 同上
  - `src/pages/CsvMapping/`: UI・ハンドラー実装
  - `src/pages/CsvMapping/utils/mappingHelpers.js`: デフォルト値設定

- **動作仕様**:
  - 項目名は固定化、0値表示は現在の設定を参照
  - 過去明細でも0値表示制御が可能
  - 新規項目はデフォルトで0値非表示
