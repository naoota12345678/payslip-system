...
exports.processCSV = functions.https.onCall(async (data, context) => {
    ...
    console.log('受信データ (RAW):', JSON.stringify(data));
    ...
    */
    ...
    // データの型が対象と異なる場合の対応を追加
    const params = typeof data === 'object' ? data : {};
    console.log('パラメータ抽出後:', JSON.stringify(params));

    const { uploadId, fileUrl, companyId, updateEmployeeInfo, columnMappings } = params;

    // 各パラメータの詳細ログを出力
    console.log('抽出したパラメータ:', {
      uploadId: uploadId || 'なし',
      fileUrl: fileUrl ? '(URLあり)' : 'なし',
      companyId: companyId || 'なし',
      updateEmployeeInfo: updateEmployeeInfo !== undefined ? updateEmployeeInfo : 'なし',
      columnMappings: columnMappings ? `(${Object.keys(columnMappings || {}).length}項目)` : 'なし'
    });
    ...
    if (updateEmployeeInfo === undefined) {
