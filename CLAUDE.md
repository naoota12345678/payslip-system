...
## PDF配信メール送信 - isActive判定の修正（2026-01-13）

### ✅ 解決した問題

**問題**: 一括個別配信で55件中4件しかメール送信されない
**原因**: `isActive` フィールドが `undefined` の従業員がスキップされていた

### 🔧 修正内容

**変更箇所**: `functions/index.js` - `sendDocumentDeliveryNotification`関数

```javascript
// 修正前（問題）
if (employeeData.email && employeeData.isActive) {
  // isActive が true の場合のみ送信
  // undefined はスキップされる
}
...
- **PDF配信**: 源泉徴収票など退職者にも送付が必要な書類があるため、全従業員に送信可能
- **給与明細**: 在職者のみに送信（退職者は対象外）
- **招待メール**: 在職者のみに送信（退職者はログイン不要）

---

**最終更新**: 2026-01-13（PDF配信isActive判定修正版）